...
def find_skyboxes(gminfopath, vpkexe):
...
# returns bitmaps
def load_sky_bitmap(skyinfo, vpkexe, vtfcmdexe):
	import subprocess, base64, json
	from pathlib import Path 
	vpkexe = Path(vpkexe)
	vtfcmdexe = Path(vtfcmdexe)
		...
		for svtf in skysides:
			...
			for hdld in hl_dict:
				...
				if tgt_vtf.with_suffix('.tga').is_file():
					...
					result_sky[svtf + '_' + hdld] = base64.b64encode(tgt_vtf.with_suffix('.tga').read_bytes()).decode()

	return json.dumps(result_sky)
